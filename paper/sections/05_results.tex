...
Table~\ref{tab:main} compares PCGT against 12 baselines on 7 medium-scale datasets. Baseline results for GCN through SGFormer are taken from \citet{wu2023sgformer}; DIFFormer results are from \citet{wu2023difformer} and NAGphormer from \citet{chen2023nagphormer}, both under the same evaluation protocol. PCGT numbers are from our own runs on an NVIDIA H100 GPU (10 runs each).
...
\caption{Node classification accuracy (\%) on medium-scale benchmarks. Mean $\pm$ std over 10 runs (Deezer: 5 runs). GCN--SGFormer from \citet{wu2023sgformer}; DIFFormer from \citet{wu2023difformer}; NAGphormer from \citet{chen2023nagphormer}. \best{Best} and \runup{runner-up} per column.}
\label{tab:main}
\end{table*}
...
\midrule
\textbf{PCGT (ours)} & \best{72.63}{\scriptsize$\pm$0.08} \\
\bottomrule
\end{tabular}
\caption{Node classification accuracy (\%) on ogbn-arxiv (169K nodes). Baselines from~\citet{wu2023sgformer} Table~3. PCGT: 3 runs, $K$=256 partitions, best validation epoch.}
\label{tab:arxiv}
\end{table}

\noindent PCGT scores 72.63\% on ogbn-arxiv, tying SGFormer (72.63\%) and beating all other baselines including DIFFormer (72.02\%) and GCN (71.74\%). On a homophilic graph like arxiv ($h$=0.66), SGFormer's lightweight $O(N)$ kernel is well-suited, so tying it is the expected outcome---what matters is that PCGT does not lose accuracy at this scale. The result shows that the $O(N^2/K + NMK)$ complexity is practical at 169K nodes.
