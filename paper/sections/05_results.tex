...
\noindent PCGT scores 72.50\% on ogbn-arxiv, within the error bars of SGFormer (72.63$\pm$0.13\%) and beating all other baselines including DIFFormer (72.02\%) and GCN (71.74\%). On a homophilic graph like arxiv ($h$=0.66), SGFormer's lightweight $O(N)$ kernel is well-suited, so matching it is the expected outcome---what matters is that PCGT does not lose accuracy at this scale. The result shows that the $O(N^2/K + NMK)$ complexity is practical at 169K nodes.
