...
GraphGPS & 78.6{\scriptsize$\pm$1.7} & 60.4{\scriptsize$\pm$1.4} & 75.5{\scriptsize$\pm$0.7} & 34.0{\scriptsize$\pm$0.6} & 36.5{\scriptsize$\pm$2.5} & 44.2{\scriptsize$\pm$3.5} & \runup{67.2}{\scriptsize$\pm$0.7} \\
SGFormer & \best{84.5}{\scriptsize$\pm$0.8} & \runup{72.6}{\scriptsize$\pm$0.2} & \runup{80.3}{\scriptsize$\pm$0.6} & \runup{37.9}{\scriptsize$\pm$1.1} & \runup{41.8}{\scriptsize$\pm$2.2} & \runup{44.9}{\scriptsize$\pm$3.9} & 67.1{\scriptsize$\pm$1.1} \\
\midrule
...
\caption{Node classification accuracy (\%) on medium-scale benchmarks. Mean $\pm$ std over 10 runs (Deezer: 5--10 runs). GCN--SGFormer from \citet{wu2023sgformer}; DIFFormer from \citet{wu2023difformer}; NAGphormer from \citet{chen2023nagphormer}; GraphGPS from our runs. \best{Best} and \runup{runner-up} per column.}
\label{tab:main}
\end{table*}
...
\midrule
GCN & 91.5{\scriptsize$\pm$0.3} & 95.7{\scriptsize$\pm$0.2} & 82.8{\scriptsize$\pm$1.8} & 90.9{\scriptsize$\pm$1.2} \\
...
\textbf{Homophilic datasets.} On CiteSeer, PCGT scores 73.1 vs.\ SGFormer's 72.6 (+0.5\%, paired $t$-test $p$=0.048). On PubMed, 81.0 vs.\ 80.3 (+0.7\%, $p$=0.027). On Cora, the two are within noise (84.3 vs.\ 84.5, $p$=0.88). PCGT also shows lower run-to-run variance than SGFormer across these benchmarks.
...
\subsection{Large-Scale Results: ogbn-arxiv}

To test scalability beyond medium-sized graphs, we evaluate on ogbn-arxiv (169K nodes, 1.2M edges), the standard large-scale benchmark for graph transformers~\citep{hu2020ogb}. We use the official OGB split and follow the mini-batch training setup of SGFormer~\citep{wu2023sgformer} with batch size 10K, hidden dimension 256, 3-layer GCN backbone, and 1 PCGT attention layer with $K$=256 partitions.
...
\midrule
GCN~\citep{kipf2017semi} & 71.74{\scriptsize$\pm$0.29} \\
SIGN~\citep{rossi2020sign} & 71.95{\scriptsize$\pm$0.11} \\
GraphSAGE~\citep{hamilton2017sage} & 71.49{\scriptsize$\pm$0.27} \\
ClusterGCN~\citep{chiang2019clustergcn} & 71.24{\scriptsize$\pm$0.44} \\
NodeFormer~\citep{wu2022nodeformer} & 59.90{\scriptsize$\pm$0.42} \\
DIFFormer~\citep{wu2023difformer} & 72.02{\scriptsize$\pm$0.13} \\
SGFormer~\citep{wu2023sgformer} & \best{72.63}{\scriptsize$\pm$0.13} \\
\midrule
\textbf{PCGT (ours)} & \runup{72.50}{\scriptsize$\pm$0.14} \\
\bottomrule
\end{tabular}
\caption{Node classification accuracy (\%) on ogbn-arxiv (169K nodes). Baselines from~\citet{wu2023sgformer} Table~3. PCGT: 10 runs, $K$=256 partitions, best validation epoch.}
\label{tab:arxiv}
\end{table}

\noindent PCGT scores 72.50\% on ogbn-arxiv, within the error bars of SGFormer (72.63$\pm$0.13\%) and above all other baselines including DIFFormer (72.02\%) and GCN (71.74\%). This small gap is expected rather than surprising: ogbn-arxiv is a homophilic citation network ($h$=0.66) where connected papers share topics, and its 128-dimensional features are already well-aligned with neighborhood structure. In this regime, SGFormer's $O(N)$ global kernel---which effectively computes a rank-1 all-pair attention---is well-matched to the task, and the additional structural encoding from PCGT's partition attention provides limited additional signal. Conversely, PCGT's largest gains occur precisely where SGFormer's topology-agnostic kernel falls short: on heterophilic graphs where intra-partition structure differs qualitatively from random mixing. The ogbn-arxiv result demonstrates that PCGT degrades gracefully on datasets outside its primary advantage regime, maintaining competitive accuracy at 169K-node scale without the catastrophic failure modes seen in other transformers (cf.\ NodeFormer at 59.9\%).
...
\begin{itemize}
\item $\alpha$ (local vs.~global blend) stabilizes near 0.5 across datasets. This indicates that both attention resolutions contribute complementary information under the current single-layer design: local attention captures fine-grained intra-partition patterns while global attention provides cross-partition context, and neither consistently dominates. Critically, ablations confirm that removing \emph{either} branch degrades accuracy (Table~\ref{tab:ablation}: $-$0.17 to $-$0.30 without global, $-$0.39 to $-$0.89 without local), so the equal blend reflects genuine contribution from both scales rather than redundancy. We note that $\alpha$ \emph{could} in principle be fixed at 0.5 with minimal performance loss; the learnable parameterization serves primarily as a safety valve that prevents pathological configurations during training.
...
Random part. & 83.80{\scriptsize$\pm$.53} & 48.80{\scriptsize$\pm$2.07} & 44.60{\scriptsize$\pm$1.80} \\
\midrule
...
\textbf{Topology-aware partitioning matters selectively.}~~Replacing METIS with random partitions hurts Squirrel ($-$0.68) more than Chameleon ($-$0.26). Although Chameleon has higher Newman modularity ($Q=0.58$ vs.\ $Q=0.38$), the relevant measure is the \emph{damage} caused by random assignment. Squirrel's denser connectivity (avg.\ degree 42 vs.\ 20) means random partitions mix more structurally dissimilar nodes, destroying the fragile-but-real community structure that METIS captures. Chameleon's smaller, sparser graph is less sensitive to partition quality.
...
Table~\ref{tab:runtime} reports per-epoch training time, single-pass inference time, and peak GPU memory on an NVIDIA H100 80GB GPU. We compare SGFormer ($O(N)$ global attention) and PCGT ($O(N^2/K + NMK)$ partition-conditioned attention plus GCN).
...
\toprule
 & & & \multicolumn{2}{c|}{\textbf{Train (ms/ep)}} & \multicolumn{2}{c|}{\textbf{Infer (ms)}} & \multicolumn{2}{c}{\textbf{Mem (MB)}} \\
\textbf{Dataset} & $N$ & $K$ & \textbf{SGF} & \textbf{PCGT} & \textbf{SGF} & \textbf{PCGT} & \textbf{SGF} & \textbf{PCGT} \\
\midrule
Cora       &  2,708  & 10  & 5.5   & 15.9  & 1.3  & 3.4   & 57   & 57  \\
CiteSeer   &  3,327  & 20  & 5.4   & 26.8  & 1.3  & 5.5   & 91   & 91  \\
Chameleon  &    890  & 10  & 4.6   & 15.1  & 1.3  & 3.3   & 52   & 52  \\
Squirrel   &  2,223  & 10  & 8.0   & 16.9  & 1.4  & 3.5   & 105  & 105 \\
Film       &  7,600  & 5   & 5.9   & 10.2  & 1.4  & 2.4   & 100  & 100 \\
PubMed     & 19,717  & 50  & 5.4   & 61.1  & 1.3  & 11.8  & 146  & 147 \\
Deezer     & 28,281  & 20  & 14.3  & 34.1  & 10.5 & 12.6  & 3631 & 3632 \\
\midrule
ogbn-arxiv & 169,343 & 256 & 56.8  & 430.2 & ---  & ---   & ---  & ---  \\
\bottomrule
\end{tabular}
}% end resizebox
\caption{Efficiency comparison on H100. Training time (ms/epoch), inference time (mean of 50 forward passes, ms), and peak GPU memory (MB). PCGT's training overhead is 1.7--11.2$\times$ over SGFormer, increasing with $K$. Inference overhead is 1.2--8.8$\times$; memory overhead is negligible ($<$1\,MB on all datasets). On ogbn-arxiv, a full 500-epoch run completes in $\sim$3.6 minutes.}
\label{tab:runtime}
\end{table}

\noindent \textbf{Overhead is moderate and predictable.} PCGT's training overhead over SGFormer ranges from 1.7$\times$ (Film, $K$=5) to 11.2$\times$ (PubMed, $K$=50), with a median of 3.0$\times$. The PubMed outlier reflects its large $K$=50, which creates $N^2/K \approx 7.8$M attention pairs. Inference overhead follows the same pattern but is smaller (1.2--8.8$\times$) since the backward pass is absent. Crucially, \textbf{memory overhead is negligible}: PCGT adds $<$1\,MB over SGFormer on all datasets, because the partition attention operates on the same feature tensors---only the attention computation pattern changes, not the storage footprint. On Deezer (28K nodes), both methods require $\sim$3.6\,GB, dominated by the 31K-dimensional input features. On ogbn-arxiv (169K nodes), PCGT takes 430\,ms/epoch vs.\ SGFormer's 57\,ms, but even at 500 epochs this completes in $\sim$3.6 minutes.
