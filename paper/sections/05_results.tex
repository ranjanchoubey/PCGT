...
To test scalability beyond medium-sized graphs, we evaluate on ogbn-arxiv (169K nodes, 1.2M edges) and pokec (1.6M nodes, 30.6M edges)---two standard large-scale benchmarks spanning homophilic and heterophilic regimes~\citep{hu2020ogb, lim2021large}. We follow the mini-batch training setup of SGFormer~\citep{wu2023sgformer}: batch size 10K (arxiv) / 100K (pokec), hidden dimension 256 (arxiv) / 64 (pokec). Baselines are from SGFormer Table~3.
...
\begin{tabular}{l|cc}
\toprule
\textbf{Method} & \textbf{ogbn-arxiv} & \textbf{pokec} \\
 & (169K, $h$=0.66) & (1.6M, $h$=0.45) \\
\midrule
MLP & 55.50{\scriptsize$\pm$0.23} & 60.15{\scriptsize$\pm$0.03} \\
GCN & 71.74{\scriptsize$\pm$0.29} & 62.31{\scriptsize$\pm$1.13} \\
SIGN & 70.28{\scriptsize$\pm$0.25} & 68.01{\scriptsize$\pm$0.25} \\
NodeFormer & 59.90{\scriptsize$\pm$0.42} & 70.32{\scriptsize$\pm$0.45} \\
SGFormer & \best{72.63}{\scriptsize$\pm$0.13} & \runup{73.76}{\scriptsize$\pm$0.24} \\
\midrule
\textbf{PCGT (ours)} & \runup{72.50}{\scriptsize$\pm$0.14} & \best{74.89}{\scriptsize$\pm$0.29} \\
\bottomrule
\end{tabular}
\caption{Large-scale node classification results (\%). ogbn-arxiv uses accuracy; pokec uses accuracy. Baselines from~\citet{wu2023sgformer} Table~3. PCGT: 3 runs, best validation epoch.}
\label{tab:largescale}
\end{table}

\noindent On pokec (1.6M nodes, $h$=0.45), PCGT achieves \textbf{74.89\%}, outperforming SGFormer by \textbf{+1.13\%}---the largest gap among the large-scale datasets. This confirms that partition-conditioned attention is most beneficial on graphs where community structure carries discriminative signal, even at million-node scale. On ogbn-arxiv ($h$=0.66), PCGT scores 72.50\%, within the error bars of SGFormer (72.63$\pm$0.13\%). On a homophilic graph, SGFormer's lightweight $O(N)$ kernel is well-suited; matching it shows that PCGT's $O(N^2/K + NMK)$ complexity does not sacrifice accuracy at scale.
