...
We learn $M$ seed query vectors $\mathbf{S} \in \mathbb{R}^{M \times d}$ that are shared across all partitions. For each partition $p$, the seeds attention-pool the partition's keys and values into $M$ representative vectors:
\begin{align}
\mathbf{R}_k^{(p)} &= \mathrm{softmax}\!\left( \frac{\mathbf{S} \, \mathbf{K}[S_p]^T}{\sqrt{d}} \right) \mathbf{K}[S_p] \label{eq:rep_k}\\
\mathbf{R}_v^{(p)} &= \mathrm{softmax}\!\left( \frac{\mathbf{S} \, \mathbf{K}[S_p]^T}{\sqrt{d}} \right) \mathbf{V}[S_p] \label{eq:rep_v}
\end{align}
where $\mathbf{R}_k^{(p)}, \mathbf{R}_v^{(p)} \in \mathbb{R}^{M \times d}$ are the key and value representatives for partition $p$. Stacking across all $K$ partitions yields $\mathbf{R}_k, \mathbf{R}_v \in \mathbb{R}^{KM \times d}$. Each node then cross-attends to all representatives:
\begin{equation}
\text{Attn}_{\text{global}} = \text{softmax}\left( \frac{\mathbf{Q} \, \mathbf{R}_k^T}{\sqrt{d}} \right) \mathbf{R}_v, \quad O(N \cdot M \cdot K)
\end{equation}

This two-step design (pool then cross-attend) is related to inducing points in Set Transformer~\citep{lee2019set}, but unlike generic inducing points, our seeds are \emph{topology-aware}: they pool from graph-partitioned node groups rather than arbitrary subsets. The learned seeds also adapt to the task, unlike the fixed kernel in SGFormer~\citep{wu2023sgformer}.
...
\end{equation}
Because $\beta$ is \emph{unconstrained}, it can become negative. On datasets where a node's own features are less informative than the attention context (e.g., Film, Deezer), this lets the model suppress the self-connection. Conversely, on feature-rich graphs like Chameleon and Squirrel, $\beta$ grows strongly positive.
