...
On 13 benchmarks---including pokec (1.6M nodes)---PCGT outperforms SGFormer by +4.1\% on Chameleon, +3.7\% on Squirrel, and +1.1\% on pokec, while also improving on homophilic datasets (CiteSeer +0.5\%, PubMed +0.7\%) and scaling to million-node graphs. PCGT ranks first or tied-first on 11 of 13 datasets. Ablations show that the attention mechanism's contribution grows with heterophily, and that each design choice---local attention, global seeds, partition encoding---adds measurable accuracy.
