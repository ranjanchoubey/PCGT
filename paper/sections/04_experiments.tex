...
\textbf{ogbn-arxiv}~\citep{hu2020ogb} is a large-scale citation network (169K nodes, 1.2M edges, 40 classes) from the Open Graph Benchmark, serving as the standard scalability test for graph transformers.
...
We report mean $\pm$ standard deviation of test accuracy across runs, selecting the epoch with highest validation accuracy (early stopping with patience 200).

\subsection{Baselines}

We compare against 12 methods spanning three categories: (1)~\emph{Standard GNNs}: GCN~\citep{kipf2017semi}, GAT~\citep{velickovic2018gat}, APPNP~\citep{klicpera2019appnp}, SIGN~\citep{rossi2020sign}; (2)~\emph{Heterophily-specific GNNs}: H$_2$GCN~\citep{zhu2020h2gcn}, CPGNN~\citep{zhu2021cpgnn}, GloGNN~\citep{li2022glognn}; (3)~\emph{Graph Transformers}: NodeFormer~\citep{wu2022nodeformer}, DIFFormer~\citep{wu2023difformer}, NAGphormer~\citep{chen2023nagphormer}, SGFormer~\citep{wu2023sgformer}. Baseline results for the original 7 datasets are taken from \citet{wu2023sgformer}, Table~2. For Coauthor and Amazon datasets, SGFormer baselines are from our own runs with identical code and hyperparameters.

\noindent\textbf{Note on GraphGPS and Exphormer.}~~GraphGPS~\citep{rampasek2022graphgps} and Exphormer~\citep{shirzad2023exphormer} are important graph transformer baselines; however, both primarily benchmark on \emph{graph-level} tasks (ZINC, LRGB, ogbg-mol*) and do not report node classification results under the semi-supervised protocol we follow (SGFormer splits). We discuss their architectural relationship to PCGT in Section~\ref{sec:nystrom} and Related Work.
