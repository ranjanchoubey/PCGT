...
# Clone repo, install deps, download data, run SGFormer baseline + PCGT
#
# Usage:
#   bash run_all_large.sh setup          # Install deps + download data
#   bash run_all_large.sh sgformer       # Run all SGFormer baselines
#   bash run_all_large.sh pcgt           # Run all PCGT experiments
...
case "$CMD" in
    ...
    *)
        ...
        echo "  1. bash run_all_large.sh setup"
        echo "  2. nohup bash run_all_large.sh all > ../logs/large_all.log 2>&1 &"
        echo "  3. bash run_all_large.sh check"
        ;;
esac
